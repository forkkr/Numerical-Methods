...
class Bisection:
   ...
   def __init__(self, x1, x2):
       if Bisection.getfunctionvalue(x1)<0:
           self.x_neg = x1
           self.x_pos = x2
       elif Bisection.getfunctionvalue(x2)<0 :
           self.x_neg = x2
           self.x_pos = x1
       else:
   ...
   def run(self):
       ...
       error = Bisection.finderror(new_mid, self.x_mid)
       self.x_mid = new_mid
       f_x = Bisection.getfunctionvalue(self.x_mid)
...
ax1 = plt.subplot()
ax1.set_ylim(-.00005,.0005)
ax1.set_xlim(0.005,.09)
plt.scatter(xm,fxm)
plt.show()
